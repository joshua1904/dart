...
def get_game_context(game: Game) -> dict:
    ...
    average_score = round(total_points_scored / round_count, 1) if round_count > 0 else 0
    return {"game": game, "rounds": rounds, "round_count": round_count, "current_score_left": current_score_left, "checkout_suggestion": checkout_suggestion, "average_score": average_score, "total_points_scored": total_points_scored}

def get_left_points(game: Game) -> int:
    total_points = game.game_rounds.all().aggregate(total_points=Sum('points'))['total_points'] or 0
