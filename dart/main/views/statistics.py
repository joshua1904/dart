...
class StatisticsView(views.View):
    def get(self, request):
        ...
        if request.META.get('HTTP_HX_REQUEST'):
            return render(request, 'statistics.detail.html', context={"games": games, 'filter': filter, 'scores': scores, 'rounds': rounds})

        return render(request, 'statistics.html', context={"games": games, 'filter': filter, 'scores': scores, 'rounds': rounds})